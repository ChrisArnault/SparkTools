...
if [ -z "$JDK" -a -s "$HOME/.IdeaIC2017.2/config/idea.jdk" ]; then
  USER_JRE=`"$CAT" $HOME/.IdeaIC2017.2/config/idea.jdk`
  if [ ! -d "$USER_JRE" ]; then
...
elif [ -r "$HOME/.IdeaIC2017.2/config/idea$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.IdeaIC2017.2/config/idea$BITS.vmoptions"
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$IDE_BIN_HOME/lib$AGENT_LIB.so" ]; then
    AGENT="-agentlib:$AGENT_LIB=disablealloc,delay=10000,probe_disable=*,sessionname=IdeaIC2017.2"
  fi
fi
...
LD_LIBRARY_PATH="$IDE_BIN_HOME:$LD_LIBRARY_PATH" "$JAVA_BIN" \
  ...
  -Didea.paths.selector=IdeaIC2017.2 \
